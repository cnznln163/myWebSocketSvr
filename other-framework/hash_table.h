...
static uint32_t hash_func_u32(char * key_string, int key_len)
{
    uint32_t hash_u32 = (uint8_t)key_string[0];
    int i = 0;
    
    for (i=1; i<key_len; i++)
    {
        hash_u32 = hash_u32*31 + (uint8_t)key_string[i];
    }
    return hash_u32;
}

static uint64_t hash_func_u64(char * key_string, int key_len)
{
    uint64_t hash_u64 = 2147483647; // (1<<31)-1
    
    for (int i=0; i<key_len; i++)
    {
        hash_u64 = hash_u64*31 + key_string[i];
    }
...
static user_data_t * get_from_hash_table(hash_table_t * p_hash_table, char * key, int key_len)
{
    ...
    key_hash = hash_func_u32(key, key_len);
